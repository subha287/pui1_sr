...
if(localStorage.getItem('cart')){
   cart = JSON.parse(localStorage.getItem('cart')); /// if cart elements exist as per memory use them and update cart variable  
}
...
//changed printonconsole funtion to change storage elements instead of printing on console like in HW 4
function printOnConsole() {
   let userSelection = new Rollprint(rollType, glazeChoice, packChoice, basePrice)
   cart.push(userSelection);
   console.log(cart);
   localStorage.setItem('cart', JSON.stringify(cart));  // save the cart elements in local storage
